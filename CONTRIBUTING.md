...
- Use **Xenon** (Radon-backed) for cyclomatic complexity on `yt_framework` and `ytjobs`. Thresholds are `--max-absolute=C`, `--max-modules=B`, `--max-average=A` (see [.pre-commit-config.yaml](.pre-commit-config.yaml) and the CI lint job); tighten them only when the codebase is ready (see [Xenon](https://github.com/rubik/xenon) and [Radon ranks](https://radon.readthedocs.io/en/latest/commandline.html#the-cc-command)).
