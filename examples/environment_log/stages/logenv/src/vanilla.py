...
def run_command(cmd, logger, description="command", timeout=10):
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            logger.warning(f"{description} failed (exit {result.returncode}): {result.stderr.strip()[:200]}")
            return None
...
def log_gpu_info(logger):
    ...
    if nvcc_output:
        logger.info(f"nvcc version: {nvcc_output.split('release')[-1].strip() if 'release' in nvcc_output else nvcc_output}")
...
def log_python_environment(logger):
    ...
    pip_output = run_command([sys.executable, "-m", "pip", "freeze"], logger, "pip freeze", timeout=30)
    if pip_output:
...
def log_system_info(logger):
    ...
    if cpu_info:
        ...
        for line in cpu_info.split('\n'):
            if any(key in line for key in ['Model name', 'CPU(s)', 'Thread', 'Core', 'Socket', 'MHz']):
...
def log_network_info(logger):
    ...
    logger.info(f"Internet connectivity (ping 8.8.8.8): {'OK' if ping_output else 'FAILED'}")
        ...
        def format_tree(path, prefix="", max_depth=3, current_depth=0, max_files=200):
            ...
            try:
                ...
                entries = [e for e in entries if not any(
                    pattern in e.name for pattern in ['__pycache__', '.pyc', '.git', '.egg-info', '.dev']
                )]
...
def log_dl_frameworks(logger):
    ...
    try:
        ...
        logger.info(f"  - CUDA: {torch.version.cuda if torch.version.cuda else 'CPU-only'}")
    ...
    try:
        ...
        logger.info(f"  - Available providers: {', '.join(ort.get_available_providers())}")
    except ImportError:
    ...
    libraries = [
        ...
        'matplotlib', 'opencv-python', 'pillow', 'transformers'
    ]
