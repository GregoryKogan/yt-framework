...
In dev mode, MapReduce and reduce run locally as subprocesses (JSONL stdin/stdout) with the same contract as command-mode map. MapReduce runs mapper → sort by `sort_by` or `reduce_by` → reducer. Reduce-only auto-sorts by `sort_by` when set (otherwise `reduce_by`) before the reducer.
